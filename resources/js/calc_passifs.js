...
function calcCP(){
    $(CP_IDS).on('blur', function(){
        const capital_social_n_1 = parseFloat(cleanNumber($('#capitaux_capital_social_n-1').val().replace(",", "."))) || 0;
        const reserves_legales_n_1 = parseFloat(cleanNumber($('#capitaux_réserves_légales_n-1').val().replace(",", "."))) || 0;
        const autres_capitaux_propres_n_1 = parseFloat(cleanNumber($('#capitaux_autres_capitaux_propres_n-1').val().replace(",", "."))) || 0;
        const reserves_speciales_n_1 = parseFloat(cleanNumber($('#capitaux_réserves_spéciales_de_réévaluation_n-1').val().replace(",", "."))) || 0;
        const resultats_reportes_n_1 = parseFloat(cleanNumber($('#capitaux_résultats_reportés_n-1').val().replace(",", "."))) || 0;
        const modifications_comptables_n_1 = parseFloat(cleanNumber($('#capitaux_modifications_comptables_n-1').val().replace(",", "."))) || 0;
        const resultats_exercice_n_1 = parseFloat(cleanNumber($('#capitaux_résultat_de_lexercice_n-1').val().replace(",", "."))) || 0;

        const capital_social_n = parseFloat(cleanNumber($('#capitaux_capital_social_n').val().replace(",", "."))) || 0;
        const reserves_legales_n = parseFloat(cleanNumber($('#capitaux_réserves_légales_n').val().replace(",", "."))) || 0;
        const autres_capitaux_propres_n = parseFloat(cleanNumber($('#capitaux_autres_capitaux_propres_n').val().replace(",", "."))) || 0;
        const reserves_speciales_n = parseFloat(cleanNumber($('#capitaux_réserves_spéciales_de_réévaluation_n').val().replace(",", "."))) || 0;
        const resultats_reportes_n = parseFloat(cleanNumber($('#capitaux_résultats_reportés_n').val().replace(",", "."))) || 0;
        const modifications_comptables_n = parseFloat(cleanNumber($('#capitaux_modifications_comptables_n').val().replace(",", "."))) || 0;
        const resultats_exercice_n = parseFloat(cleanNumber($('#capitaux_résultat_de_lexercice_n').val().replace(",", "."))) || 0;
...
function calcPNC(){
    $(PNC_IDS).on('blur', function(){
        const emprunts_n_1 = parseFloat(cleanNumber($('#passifs_emprunts_n-1').val().replace(",", "."))) || 0;
        const provisions_n_1 = parseFloat(cleanNumber($('#passifs_provisions_n-1').val().replace(",", "."))) || 0;
        const autres_passifs_fin_n_1 = parseFloat(cleanNumber($('#passifs_autres_passifs_financiers_n-1').val().replace(",", "."))) || 0;

        const emprunts_n = parseFloat(cleanNumber($('#passifs_emprunts_n').val().replace(",", "."))) || 0;
        const provisions_n = parseFloat(cleanNumber($('#passifs_provisions_n').val().replace(",", "."))) || 0;
        const autres_passifs_fin_n = parseFloat(cleanNumber($('#passifs_autres_passifs_financiers_n').val().replace(",", "."))) || 0;
...
function calcPC(){
    $(PC_IDS).on('blur', function(){
        const fournisseurs_n_1 = parseFloat(cleanNumber($('#passifs_fournisseurs_et_comptes_rattachés_n-1').val().replace(",", "."))) || 0;
        const autres_passifs_c_n_1 = parseFloat(cleanNumber($('#passifs_autres_passifs_courants_n-1').val().replace(",", "."))) || 0;
        const concours_n_1 = parseFloat(cleanNumber($('#passifs_concours_bancaires_et_autres_passifs_financiers_n-1').val().replace(",", "."))) || 0;

        const fournisseurs_n = parseFloat(cleanNumber($('#passifs_fournisseurs_et_comptes_rattachés_n').val().replace(",", "."))) || 0;
        const autres_passifs_c_n = parseFloat(cleanNumber($('#passifs_autres_passifs_courants_n').val().replace(",", "."))) || 0;
        const concours_n = parseFloat(cleanNumber($('#passifs_concours_bancaires_et_autres_passifs_financiers_n').val().replace(",", "."))) || 0;
